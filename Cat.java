	...
	public static Cat createInstance(InstanceType type, int id, String name) {
		switch (type) {
			...
			case MOCK:
				...
				Cat mocking = Mockito.mock(Cat.class); 
				when(mocking.getId()).thenReturn(id);
				when(mocking.getName()).thenReturn(name);
				when(mocking.getRented()).thenReturn(false);
				when(mocking.toString()).thenReturn("ID " + id + ". " + name);
				return mocking;
			default:
				assert(false);
				return null;
